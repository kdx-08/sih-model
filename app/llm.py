...
def _fetch_tavily_news(lat: float, lon: float, api_key: str):
    ...
    try:
        ...
        # Create multiple specific queries for tourism safety and crime
        tourism_queries = [
            f"tourist safety crime incidents {location_name}",
            f"travel advisory warning {location_name} tourism",
            f"tourist attacked robbery theft {location_name}",
            f"tourism security concerns {location_name}",
            f"visitor safety incidents {location_name}",
            f"travel warning crime {location_name}",
        ]

        all_articles = []

        # Search with different queries to get comprehensive coverage
        for query in tourism_queries[:3]:  # Limit to 3 queries to avoid rate limits
            try:
                ...
                for result in response.get("results", []):
                    ...
                    # Check for tourism/safety related keywords
                    tourism_keywords = [
                        "tourist",
                        "tourism",
                        "travel",
                    ...
                    if relevance_score >= 2:  # Must have at least 2 relevant keywords
                        all_articles.append(
                            {
                ...
                print(f"Error in query '{query}': {str(query_error)}")
                continue
        ...
        for article in sorted(
            all_articles, key=lambda x: x.get("relevance_score", 0), reverse=True
        ):
            ...
            for seen_title in seen_titles:
                ...
                if (
                    len(title_words & seen_words)
                    / max(len(title_words), len(seen_words))
                    > 0.6
                ):
                    is_duplicate = True
                    break

            if not is_duplicate and len(unique_articles) < 5:
        ...
        return {
            ...
            "focus": "tourism safety, crime, travel advisories",
        }
